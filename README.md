...
2. Set nonempty values for `DB_PASSWORD`, `MYSQL_ROOT_PASSWORD`, `RABBITMQ_USER`, and `RABBITMQ_PASSWORD` in `.env`. You may change `APP_PORT` and `RABBITMQ_MANAGEMENT_PORT` if those host ports are occupied.
